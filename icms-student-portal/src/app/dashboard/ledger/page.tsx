  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#0B1120] flex flex-col items-center justify-center p-4 space-y-4">
        ...
        <p className="text-amber-400 font-bold uppercase tracking-widest text-xs animate-pulse">Decrypting Ledger...</p>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-8 pt-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 animate-in slide-in-from-top-4 fade-in duration-500">
          ...
          <div className="bg-white/5 border border-white/10 backdrop-blur-md px-6 py-4 rounded-2xl text-center md:text-right">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Lifetime Earned</p>
            <p className="text-3xl font-black text-amber-400">{totalXp} <span className="text-lg text-amber-400/50">XP</span></p>
          </div>
        </div>
        ...
        {transactions.length === 0 ? (
          <div className="text-center p-12 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-md animate-in fade-in duration-700">
            ...
            <p className="text-sm text-slate-500 mt-2">Complete quests and attend classes to earn XP!</p>
          </div>
        ) : (
          <div className="relative border-l-2 border-slate-800 ml-4 md:ml-6 pl-6 md:pl-8 space-y-8">
            {transactions.map((tx, index) => {
              ...
              const borderTheme = isPositive ? "border-amber-400/30" : "border-red-400/30";
              ...
              return (
                ...
                >
                  {/* Timeline Node */}
                  <div className={`absolute -left-[35px] md:-left-[41px] top-2 w-4 h-4 rounded-full border-4 border-[#020617] ${isPositive ? 'bg-amber-400' : 'bg-red-400'} shadow-[0_0_10px_rgba(251,191,36,0.5)]`}></div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">
                          {new Date(tx.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' })}
                        </p>
                      </div>

                      <div className={`shrink-0 px-4 py-2 rounded-xl border ${borderTheme} ${bgTheme} flex items-center justify-center`}>
